...
namespace DMS.API.Controllers.MD
{
    ...
    [ApiController]
    public class ChipperController : ControllerBase
    {
        public readonly IChipperService _service;
        public ChipperController(IChipperService service)
        {
            _service = service;
        }
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblChipperDto chipper)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(chipper);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblChipperDto chipper)
        {
            var transferObject = new TransferObject();
            await _service.Update(chipper);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;

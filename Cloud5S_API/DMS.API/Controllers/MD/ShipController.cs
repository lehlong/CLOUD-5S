...
namespace DMS.API.Controllers.MD
{
    ...
    public class ShipController : ControllerBase
    {
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblShipDto Ship)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(Ship);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblShipDto Ship)
        {
            var transferObject = new TransferObject();
            await _service.Update(Ship);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
        ...
        public async Task<IActionResult> Export([FromQuery] BaseExportFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DSTau" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;

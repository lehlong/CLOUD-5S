...
namespace DMS.API.Controllers.MD
{
    ...
    public class ItemController : ControllerBase
    {
        ...
        [HttpGet("Export")]
        public async Task<IActionResult> Export([FromQuery] ItemFilterLite filter)
        {
            var transferObject = new TransferObject();
            var result = await _service.Export(filter);
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DSSanPham" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblItemCreateDto item)
        {
            var transferObject = new TransferObject();

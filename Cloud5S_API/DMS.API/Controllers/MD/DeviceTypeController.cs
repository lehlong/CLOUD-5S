...
namespace DMS.API.Controllers.MD
{
    ...
    {
        public readonly IDeviceTypeService _service;
        public DeviceTypeController(IDeviceTypeService service)
        {
            _service = service;
        }

        [CustomAuthorize(Right = "R4.1.1")]
        [HttpGet("Search")]
        ...
        [CustomAuthorize(Right = "R4.1.2")]
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblDeviceTypeLiteDto area)
        {
            var transferObject = new TransferObject();
        ...
        [CustomAuthorize(Right = "R4.1.3")]
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblDeviceTypeLiteDto area)
        {
            var transferObject = new TransferObject();
        ...
        public async Task<IActionResult> Export([FromQuery] BaseExportFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DSLOAI_THIETBI" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;

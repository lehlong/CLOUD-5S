...
namespace DMS.API.Controllers.MD
{
    ...
    {
        public readonly IWorkingShiftService _service;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblWorkingShiftDto WorkingShift)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(WorkingShift);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblWorkingShiftDto WorkingShift)
        {
            var transferObject = new TransferObject();
            await _service.Update(WorkingShift);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
        ...
        public async Task<IActionResult> Export([FromQuery] BaseExportFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DSCaLamViec" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;

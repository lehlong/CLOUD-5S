...
namespace DMS.API.Controllers.MD
{
    ...
    {
        public readonly IDeviceGroupService _service;
        public DeviceGroupController(IDeviceGroupService service)
        {
            _service = service;
        }

        [CustomAuthorize(Right = "R4.2.1")]
        [HttpGet("Search")]
        ...
        [CustomAuthorize(Right = "R4.2.2")]
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblDeviceGroupLiteDto area)
        {
            var transferObject = new TransferObject();
        ...
        [CustomAuthorize(Right = "R4.2.3")]
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblDeviceGroupLiteDto area)
        {
            var transferObject = new TransferObject();
        ...
        public async Task<IActionResult> Export([FromQuery] BaseExportFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DSNHOM_THIETBI" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;

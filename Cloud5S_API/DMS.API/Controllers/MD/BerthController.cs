...
namespace DMS.API.Controllers.MD
{
    ...
    {
        public readonly IBerthService _service;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblBerthCreateUpdateDto Berth)
        {
            var transferObject = new TransferObject();
            var result = await _service.Add(Berth);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblBerthCreateUpdateDto Berth)
        {
            var transferObject = new TransferObject();
            await _service.Update(Berth);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
        ...
        public async Task<IActionResult> Export([FromQuery] BaseExportFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DSCauCang" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;

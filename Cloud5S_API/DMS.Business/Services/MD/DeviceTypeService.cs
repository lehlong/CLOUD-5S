...
namespace DMS.BUSINESS.Services.MD
{
    ...
    {
        Task<IList<tblDeviceTypeDto>> GetAll(BaseMdFilter filter);
    ...
    public class DeviceTypeService : GenericService<tblMdDeviceType, tblDeviceTypeDto>, IDeviceTypeService
    {
        ...
        public async Task<IList<tblDeviceTypeDto>> GetAll(BaseMdFilter filter)
        {
            try
            {
                var query = this._dbContext.tblMdDeviceType.AsQueryable();
        ...
        public async Task<byte[]> Export(BaseExportFilter filter)
        {
            try
            {
                ...
                var data = raw_data.Select((x, i) => new tblDeviceTypeDto()
                {
                    OrdinalNumber = i + 1,
                    Code = x.Code,
                    IsActive = x.IsActive,
                ...
                var result = await new ExcelExporter(_dbContext).ExportToExcel(data, Common.Enum.ExcelExportType.DSLOAI_THIETBI);

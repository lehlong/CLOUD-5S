...
namespace DMS.BUSINESS.Services.MD
{
    ...
    {
        Task<IList<tblDepartmentDto>> GetAll(BaseMdFilter filter);
    ...
    public class DepartmentService : GenericService<tblMdDepartment, tblDepartmentDto>, IDepartmentService
    {
        ...
        public override async Task<PagedResponseDto> Search(BaseFilter filter)
        {
            try
            {
                var query = this._dbContext.tblMdDepartment.Include(x=>x.Company).AsQueryable();
        ...
        public async Task<IList<tblDepartmentDto>> GetAll(BaseMdFilter filter)
        {
            try
            {
                ...
                query = query.OrderByDescending(x => x.CreateDate);
                return _mapper.Map<IList<tblDepartmentDto>>(await query.ToListAsync());
            }
            catch (Exception ex)
            {
                this.Status = false;
        ...
        public async Task<byte[]> Export(BaseExportFilter filter)
        {
            try
            {
                ...
                var data = raw_data.Select((x, i) => new tblDepartmentDto()
                {
                    OrdinalNumber = i + 1,
                    Code = x.Code,
                    IsActive = x.IsActive,

...
namespace DMS.BUSINESS.Services.MD
{
    ...
    {
        Task<IList<tblUnitDto>> GetAll(BaseMdFilter filter);
    ...
    public class UnitService : GenericService<tblMdUnit, tblUnitDto>, IUnitService
    {
        ...
        public async Task<IList<tblUnitDto>> GetAll(BaseMdFilter filter)
        {
            try
            {
                var query = this._dbContext.tblMdUnit.AsQueryable();
        ...
        public async Task<byte[]> Export(BaseExportFilter filter)
        {
            try
            {
                ...
                var data = raw_data.Select((x, i) => new tblUnitDto()
                {
                    OrdinalNumber = i + 1,
                    Code = x.Code,
                    IsActive = x.IsActive,
                ...
                var result = await new ExcelExporter(_dbContext).ExportToExcel(data, Common.Enum.ExcelExportType.DON_VI_TINH);

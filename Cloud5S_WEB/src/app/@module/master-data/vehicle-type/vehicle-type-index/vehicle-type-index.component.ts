...
import {VehicleTypeFilter, optionsGroup} from 'src/app/@filter/MD/vehicle-type.filter';
...
export class VehicleTypeIndexComponent {
  constructor(
    ...
    this.globalService.setBreadcrumb([
      {
        name: 'Danh sách loại phương tiện',
        path: 'master-data/vehicle-type',
      },
    ]);
  ...
  exportExcel() {
    return this._service.ExportExcel(this.filter).subscribe((result: Blob) => {
      ...
      anchor.download = 'danh-sach-loai-phuong-tien.xlsx';
      anchor.href = url;
      anchor.click();
    });
  }
  ...
  openCreate() {
    this.drawerService.open(VehicleTypeCreateComponent).subscribe((result) => {
      if (result?.status && this.utils.checkComponent(VehicleTypeIndexComponent)) {
        this.loadInit();
      }
    });
  }
